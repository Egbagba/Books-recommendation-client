  ...
  return (
    <nav className='mb-5 py-2 px-2 fixed top-0 left-0'>
      <div className='text-gray bg-ivory navbar space-x-4'>
        <Link to="/"><button className='btn btn-ghost text-l text-slate-600 font-bold font-serif'>Home</button></Link>

        {isLoggedIn && (
          <>
            <Link to="/About"><button className='text-gray font-bold font-serif btn btn-ghost text-l text-slate-600'>About</button></Link>

            {/*   UPDATE   */}
            <button className='text-gray font-bold font-serif btn btn-ghost text-l text-slate-600' onClick={handleLogout}>Logout</button>
          </>
        )}
